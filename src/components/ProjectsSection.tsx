    ...
    >
      <div className={`${cardHeight} glass-effect tech-pattern matrix-overlay tech-border glow-effect cyber-pulse
        ...
        >
          <div>
            <motion.h3 
              className={`text-2xl font-bold mb-3 bg-gradient-to-r ${gradient} bg-clip-text text-transparent`}
...
const ProjectsSection = () => {
  ...
  const infiniteProjects = [...projects, ...projects, ...projects, ...projects].filter(p => p.featured);
            ...
            >
              {infiniteProjects.map((project, index) => (
                <ProjectCard 
                  ...
                  onLeave={startAnimation}
                  index={index}
                />
              ))}
            </motion.div>
          ) : (
            <div className="text-center text-gray-400">No featured projects available</div>
          )}
        </div>
      </div>
    </section>
